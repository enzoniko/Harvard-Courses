	...
	void Start () {
		...
		for (int z = 0; z < mazeSize; z++) {
			for (int x = 0; x < mazeSize; x++) {
				if (mapData[z, x]) {
					CreateChildPrefab(wallPrefab, wallsParent, x, 0, z);
					...
					// create floor and ceiling for the player
					...
					// create floor and ceiling only in places that we can see (for performance)
					CreateChildPrefab(floorPrefab, floorParent, x, 0, z);
				}
