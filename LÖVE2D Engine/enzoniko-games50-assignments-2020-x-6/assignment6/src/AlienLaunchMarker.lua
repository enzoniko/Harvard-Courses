...
function AlienLaunchMarker:update(dt)
    ...
    elseif self.launched then
        ...
        if love.keyboard.wasPressed('space') and self.alien2 == nil and self.alien3 == nil and (math.abs(xVel) + math.abs(yVel) > 300) and self.collided == false then
            local xPos, yPos = self.alien.body:getPosition()

            self.alien2 = Alien(self.world, 'round', xPos, yPos, 'Player')

            -- apply the difference between current X,Y and base X,Y as launch vector impulse
            self.alien2.body:setLinearVelocity((self.baseX - self.shiftedX) * 10, (self.baseY - self.shiftedY) * 11)

            -- make the alien pretty bouncy
            self.alien2.fixture:setRestitution(0.4)
            self.alien2.body:setAngularDamping(2)

            self.alien3 = Alien(self.world, 'round', xPos, yPos, 'Player')

            -- apply the difference between current X,Y and base X,Y as launch vector impulse
            self.alien3.body:setLinearVelocity((self.baseX - self.shiftedX) * 10, (self.baseY - self.shiftedY) * 9)

            -- make the alien pretty bouncy
            self.alien3.fixture:setRestitution(0.4)
            self.alien3.body:setAngularDamping(2)
        end
    end
end
...
function AlienLaunchMarker:render()
    if not self.launched then
        ...
        if self.alien2 and self.alien3 then
            self.alien2:render()
