...
function PlayState:update(dt)
    -- If is not possible to make a match, change board
    if self.board:matchesAreImpossible() then
    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                :finish(function()
                    -- if the movement makes a match calls calculateMatches
                    if self.board:calculateMatches() then
                        self:calculateMatches()
                    -- if the movement does not make a match, undo the movement
                    else
                        gSounds['error']:play()
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            ...
            for i, tile in pairs(match) do
                -- add a second for each tile matched
                self.timer = self.timer + 1
                self.score = self.score + tile.variety * 10
                
                -- if the tile matched was shiny, breaks all tiles in the row
