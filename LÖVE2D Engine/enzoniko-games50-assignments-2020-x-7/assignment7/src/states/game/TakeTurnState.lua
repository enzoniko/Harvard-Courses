            ...
            Timer.after(1.5, function()
                ...
                :finish(function()
                    ...
                    if self.playerPokemon.currentExp > self.playerPokemon.expToLevel then
                        ...
                        -- get current stats
                        --[[ 
                        local cHP = self.playerPokemon.HP
                        local cAttack = self.playerPokemon.attack
                        local cDefense = self.playerPokemon.defense
                        local cSpeed = self.playerPokemon.speed 
                        ]]
                        local currentStats = {self.playerPokemon.HP, self.playerPokemon.attack, self.playerPokemon.defense, self.playerPokemon.speed}
                        ...
                        -- get change in stats from levelUp()
                        local iHP, iAttack, iDefense, iSpeed = self.playerPokemon:levelUp()
                        local incrementStats = {iHP, iAttack, iDefense, iSpeed}
                        
                        gStateStack:push(BattleMessageState('Congratulations! Level Up!',
                        function()
                            -- spawn BattleMenuState with selection modified showing X + Y = Z for each stat
                            gStateStack:push(LevelUpMenuState(currentStats, incrementStats))
